...
class liner {
  ...
  void draw () {
    ...
    for (int i = 0; i < 211; i+=11) {
      ...
      //noFill();
      scale(1);
      fill(noise(frameCount*PI*0.003)* 50+200, 255, 255);
      //fill(255-(i/4)-10 % 255, 255, 250);  
      //noStroke();
      translate(x1(t+i), y1(t+i));
      //shearY(noise(frameCount*PI*0.0003)*5);
      //rotateX(noise(frameCount*PI/ 300-i)*1);
      rotateY(noise(frameCount*PI/ 900-i)*5);
      //rotateZ(noise(frameCount*PI/ 500-i)*3);
      box(600, size, size);
      //translate(x24t+i), y2(t+i));
    ...
    size = noise(frameCount * PI* 0.002) * 30;
    //strokeWeight(7);
    //box(450, size, size);
    t+=0.1; 
    //incro+=0.1;
