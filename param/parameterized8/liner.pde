...
class liner {
  ...
  void draw () {
    ...
    if (trail > 422) trail = 0;
    if (random(100)>99) rot = !rot;
    for (int i = 0; i < trail; i+=11) {
      pushMatrix();
      noStroke();
      //stroke(200+i % 255);
      pushMatrix();
      if (rot) {
      rotateY(frameCount*PI*0.001-i);
      } else {
      rotateX(frameCount*PI*0.001-i);
      }
      //rotateZ(frameCount*PI*0.001);
      stroke(255-(i/8)-50 % 255, 255, 255);  
      rect(x1(t+i), y1(t+i), 400, 10);
      rect(x2(t+i), y2(t+i), 400, 10);
      popMatrix();
      fill(0);
